...
OUTFILE=test.ic.handlers.out
REFFILE=test.ic.handlers.ref
#CKPT="--checkpoint-simulation-period=12us"
CKPT=""
TEST="Test IC set handler commands - complete"
...
echo "cd cp0" > $pipe
echo "ls" > $pipe
echo "trace size changed : 8 2 : size : interactive" > $pipe 
echo "printWatchpoint 0" > $pipe
echo "run" > $pipe
echo "printTrace 0" > $pipe

echo "setHandler 0 ac ae" > $pipe
echo "printWatchpoint 0" > $pipe
echo "run" > $pipe
echo "printTrace 0" > $pipe

echo "setHandler 0 bc be" > $pipe
echo "printWatchpoint 0" > $pipe
echo "run" > $pipe
echo "printTrace 0" > $pipe

echo "setHandler 0 ae" > $pipe
echo "printWatchpoint 0" > $pipe
echo "run" > $pipe
echo "printTrace 0" > $pipe

echo "setHandler 0 all" > $pipe
echo "printWatchpoint 0" > $pipe
echo "run" > $pipe
echo "printTrace 0" > $pipe
