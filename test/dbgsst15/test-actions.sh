...
OUTFILE=test.ic.actions.out
REFFILE=test.ic.actions.ref
CKPT=--checkpoint-sim-period=12us
#CKPT=""
TEST="Test IC set action commands - complete"
...
echo "cd cp0" > $pipe
echo "ls" > $pipe
echo "trace maxData > 10 : 8 0 : maxData : interactive" > $pipe 
echo "run" > $pipe
echo "printTrace 0" > $pipe

echo "unwatch 0" > $pipe
echo "trace maxData > 10 : 8 0 : maxData : printTrace" > $pipe
echo "run 1us" > $pipe

echo "unwatch 0" > $pipe
echo "trace maxData > 10 : 8 0 : maxData : printStatus" > $pipe
echo "run 1us" > $pipe

echo "unwatch 0" > $pipe
echo "ls" > $pipe
echo "trace maxData > 10 : 8 0 : minData : set minData 10" > $pipe
echo "run 1us" > $pipe
echo "ls" > $pipe

#echo "unwatch 0" > $pipe
#echo "trace maxData > 10 : 8 0 : maxData : checkpoint" > $pipe
#echo "run 1us" > $pipe
