...
OUTFILE=test.ic.trace.out
REFFILE=test.ic.trace.ref
#CKPT="--checkpoint-simulation-period=12us"
CKPT=""
TEST="Test IC trace commands - complete"
...
echo "cd cp0" > $pipe
echo "ls" > $pipe
echo "trace size > 50 : 32 4 : size : interactive" > $pipe 
echo "printWatchpoint 0" > $pipe
echo "addTraceVar 0 rCheck" > $pipe
echo "printWatchpoint 0" > $pipe
echo "run" > $pipe
echo "printTrace 0" > $pipe
echo "unwatch 0" > $pipe
echo "trace size changed : 10 2 : size rCheck : interactive" > $pipe
echo "run" > $pipe
echo "printTrace 0" > $pipe
echo "unwatch 0" > $pipe
echo "trace minData < size : 8 0 : size : interactive" > $pipe
echo "run" > $pipe
echo "printTrace 0" > $pipe
echo "unwatch 0" > $pipe
echo " trace size changed && maxData > 90 || minData < maxData || rCheck changed : 4 2 : rCheck size minData maxData : interactive" > $pipe
echo "run" > $pipe
echo "printTrace 0" > $pipe
echo "shutdown" > $pipe
